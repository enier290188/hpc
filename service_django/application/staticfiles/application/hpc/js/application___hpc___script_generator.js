...
var hpc_script_init = function(){
    ...
    var compose = function() {
        ...
        if (job_name !== '' || !export_variables || partition !== '') {
            ...
            if (job_name !== '') script_slurm += '#SBATCH --job-name=' + job_name + rc;
        ...
        if (nodes !== '' || ntasks !== '' || tasks_per_node !== '' || cpus_per_task !== '' || mem !== '' && parseInt(mem) > 0
            ...
            if (nodes !== '') script_slurm += '#SBATCH --nodes=' + nodes + rc;
            ...
            if (time !== '') script_slurm += '#SBATCH --time=' + time + rc;
            if (test) script_slurm += '#SBATCH --test-only' + rc;
            if (require_gpus) script_slurm += '#SBATCH --gres=gpu:2' + rc;
            if (nodelist !== '') script_slurm += '#SBATCH --nodelist=' + nodelist + rc;
            if (exclude !== '') script_slurm += '#SBATCH --exclude=' + exclude + rc;
        }

        if (output !== '' || error !== '') {
            script_slurm += rc + "### Stream options ###" + rc;
            if (output !== '') script_slurm += '#SBATCH --output=' + output + rc;
            if (error !== '') script_slurm += '#SBATCH --error=' + error + rc;
        }
        ...
        script_slurm += rc + "### Bash script ###" + rc + cmEditor.getValue() + rc + "exit 0" + rc;
